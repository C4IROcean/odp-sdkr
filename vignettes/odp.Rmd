...
# tib     <- cursor$all_tibble()   # tibble support is optional
```
...
Datasets can also hold raw files—CSVs, Parquet, or any other format. Access the
file API through `dataset$files` (a shortcut for `dataset$table$raw`).

```{r files, eval = FALSE}
# Upload a file (character string or raw vector)
file_id <- dataset$files$upload("data.csv", "lat,lon,depth\n59.5,5.3,120\n")

# Or upload from disk
raw_bytes <- readBin("local.parquet", "raw", file.info("local.parquet")$size)
file_id2 <- dataset$files$upload("local.parquet", raw_bytes)
...
# Ingest the file into the table
# Modes: "append" (default), "truncate", or "drop"
dataset$files$ingest(file_id, opt = "append")

# Update metadata
dataset$files$update_meta(file_id, list(source = "field_survey"))
...
- Keep your `ODP_API_KEY` secret—never commit it to version control
