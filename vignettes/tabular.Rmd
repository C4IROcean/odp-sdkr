...
```{r txn, eval = FALSE}
df_new <- data.frame(depth = c(250L, 300L), stringsAsFactors = FALSE)
resp <- tbl$client$request_json(
  path = "/api/table/v2/begin",
  query = list(table_id = tbl$id),
  method = "POST"
)

tx <- OdpTransaction$new(tbl, resp$tx_id)
tx$insert(df_new)
tx$delete(query = "depth < 50")
tx$commit()
```

The SDK buffers rows and auto-flushes at 10,000 rows or 10 MB. Validation
catches schema mismatches early with helpful error messages.
